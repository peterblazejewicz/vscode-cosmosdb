...
export class CosmosDBExplorer implements TreeDataProvider<INode> {
	...
	constructor(private azureAccount: AzureAccount, private _graphView: GraphView, globalState: Memento) {
	...
	async getChildren(node?: INode): Promise<INode[]> {
		...
		if (node) {
			...
			if (this.azureAccount.status === "Initializing" || this.azureAccount.status === "LoggingIn") {
				...
				nodes = this.azureAccount.filters.map(filter => new SubscriptionNode(this._graphView, filter))
			}
